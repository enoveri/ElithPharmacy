...
function ViewProduct() {
  ...
  const [productAnalytics, setProductAnalytics] = useState({
    salesHistory: [
      { date: "2024-01-20", quantity: 5, revenue: 127.50, profit: 37.50 },
      { date: "2024-01-19", quantity: 3, revenue: 76.50, profit: 22.50 },
      { date: "2024-01-18", quantity: 8, revenue: 204.00, profit: 60.00 },
      { date: "2024-01-17", quantity: 2, revenue: 51.00, profit: 15.00 },
      { date: "2024-01-16", quantity: 6, revenue: 153.00, profit: 45.00 },
    ],
    totalSales: 24,
    totalRevenue: 612.00,
    totalProfit: 180.00,
    ...
    topCustomers: [
      { name: "John Doe", purchases: 8, totalSpent: 204.00 },
      { name: "Sarah Johnson", purchases: 6, totalSpent: 153.00 },
      { name: "Michael Brown", purchases: 5, totalSpent: 127.50 },
    ],
    stockMovement: {
      received: 200,
      sold: 24,
  ...
  if (!product) {
    return (
      <div style={{
        ...
        <FiPackage size={64} style={{ color: '#9ca3af', marginBottom: '16px' }} />
  ...
  const getStockStatus = () => {
    if (product.quantity === 0) return { color: "#ef4444", text: "Out of Stock", bg: "#fef2f2" };
    if (product.quantity <= product.minStockLevel) return { color: "#f59e0b", text: "Low Stock", bg: "#fffbeb" };
  ...
  const getExpiryStatus = () => {
    ...
    if (daysUntilExpiry <= 0) return { color: "#ef4444", text: "Expired", bg: "#fef2f2" };
    if (daysUntilExpiry <= 30) return { color: "#f59e0b", text: `${daysUntilExpiry} days left`, bg: "#fffbeb" };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontWeight: '600', color: '#1f2937' }}>{product.category}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: '#6b7280' }}>Manufacturer:</span>
            <span style={{ fontWeight: '600', color: '#1f2937' }}>{product.manufacturer}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: '#6b7280' }}>Batch Number:</span>
            <span style={{ fontWeight: '600', color: '#1f2937' }}>{product.batchNumber}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: '#6b7280' }}>Barcode:</span>
            <span style={{ fontWeight: '600', color: '#1f2937' }}>{product.barcode}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
          <div style={{ textAlign: 'center', padding: '16px', backgroundColor: '#f0fdf4', borderRadius: '8px' }}>
            <FiShoppingCart size={24} style={{ color: '#10b981', marginBottom: '8px' }} />
          ...
          <div style={{ textAlign: 'center', padding: '16px', backgroundColor: '#eff6ff', borderRadius: '8px' }}>
            <FiDollarSign size={24} style={{ color: '#3b82f6', marginBottom: '8px' }} />
            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#1f2937' }}>
          ...
          <div style={{ textAlign: 'center', padding: '16px', backgroundColor: '#fefce8', borderRadius: '8px' }}>
            <FiTrendingUp size={24} style={{ color: '#f59e0b', marginBottom: '8px' }} />
            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#1f2937' }}>
              ₦{productAnalytics.totalProfit.toFixed(2)}
            </div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Total Profit</div>
          </div>
          <div style={{ textAlign: 'center', padding: '16px', backgroundColor: '#f3e8ff', borderRadius: '8px' }}>
            <FiBarChart size={24} style={{ color: '#8b5cf6', marginBottom: '8px' }} />
            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#1f2937' }}>
              {productAnalytics.stockMovement.turnoverRate}%
            </div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>Turnover Rate</div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          {productAnalytics.demandTrend === 'increasing' ? (
            <FiTrendingUp size={48} style={{ color: '#10b981', marginBottom: '12px' }} />
          ) : (
            <FiTrendingDown size={48} style={{ color: '#ef4444', marginBottom: '12px' }} />
          )}
          <div style={{ fontSize: '16px', fontWeight: '600', color: '#1f2937', marginBottom: '4px' }}>
            {productAnalytics.demandTrend === 'increasing' ? 'Increasing' : 'Decreasing'}
          </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #f3f4f6' }}>
              ...
              <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px', fontWeight: '600' }}>Quantity Sold</th>
              <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px', fontWeight: '600' }}>Revenue</th>
              <th style={{ padding: '12px', textAlign: 'left', fontSize: '14px', fontWeight: '600' }}>Profit</th>
            </tr>
          </thead>
          <tbody>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: '14px', fontWeight: '600', color: stockStatus.color }}>
              {stockStatus.text}
            </div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>
              Current stock level
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: '14px', fontWeight: '600', color: expiryStatus.color }}>
              {expiryStatus.text}
            </div>
            <div style={{ fontSize: '12px', color: '#6b7280' }}>
