...
# Check if on Linux and run shell script instead
elif platform.system() == "Linux":
    try:
        print("Detected Linux system. Running shell script instead...")
        # Make sure the script is executable
        subprocess.run(["chmod", "+x", "./start_pharmacy_app.sh"], check=False)
        subprocess.run(["./start_pharmacy_app.sh"], check=True)
        sys.exit(0)
    except Exception as e:
...
def check_internet_connectivity():
    ...
    try:
        ...
        if result == 0:
            ...
            log("No internet connectivity detected", "WARNING")
            return False
    except Exception as e:
        log(f"Error checking internet connectivity: {e}", "WARNING")
        return False
    ...
    try:
        ...
        while not success and retry_count < max_retries:
            ...
            if "Started supabase local development setup" in result.stdout or "Started supabase local development setup" in result.stderr:
                ...
                # Check for specific error related to deno.land downloads
                output = result.stdout + result.stderr
                if "error sending request for url" in output and "deno.land" in output:
                    deno_error_detected = True
                    log("Detected Supabase edge runtime download error", "WARNING")
                    
                    # Check internet connectivity since this is likely the issue
                    if not check_internet_connectivity():
                        log("Internet connectivity issue detected", "ERROR")
                        log("Supabase edge runtime cannot download required dependencies", "ERROR")
                        log("Please connect to the internet and try again", "ERROR")
                        return False
                    else:
                        log("Internet seems to be working, but Supabase still can't download dependencies", "WARNING")
                        log("This might be due to a temporary network issue or firewall restriction", "WARNING")
        ...
        if not success:
            ...
            if deno_error_detected:
                log("The error appears to be related to downloading dependencies from deno.land", "ERROR")
                log("Please ensure you have a stable internet connection and try again", "ERROR")
            return False
