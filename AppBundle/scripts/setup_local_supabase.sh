...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --initData            Initialize database with data dump"
            echo ""
            echo "Force options (override completed steps):"
            echo "  --force-install       Force npm install step"
            echo "  --force-init          Force supabase init step"
            echo "  --force-login         Force supabase login step"
            echo "  --force-link          Force supabase link step"
            echo "  --force-start         Force supabase start step"
            echo "  --force-pull          Force database pull step"
            echo "  --force-reset         Force database reset step"
            echo "  --force-data          Force data initialization step"
            echo "  --force-all           Force all steps to run"
            echo ""
...
invoke_step() {
    ...
    local force_flag="${6:-false}"
    
    # Check if step already completed and not forced
    if [ "$(get_config_value "$config_key")" = "true" ] && [ "$force_flag" = "false" ] && [ "$FORCE_ALL" = "false" ]; then
        echo -e "\e[32m✅ Step '$name' already completed. Skipping...\e[0m"
        return 0
    fi
    
    # If step is forced, let the user know
    if [ "$force_flag" = "true" ] || [ "$FORCE_ALL" = "true" ]; then
        echo -e "\e[33m⚠️ Forcing step '$name' to run even though it was previously completed.\e[0m"
    fi
...
handle_data_init() {
    echo -e "\e[36m🔄 Initializing database with data...\e[0m"
    ...
    echo -e "\e[36m🔄 Dumping data from remote database...\e[0m"
    dump_output=$(npx supabase db dump --data-only -f supabase/data.sql 2>&1)
    dump_exit=$?
    
    if [ $dump_exit -ne 0 ]; then
        echo -e "\e[31m❌ Failed to dump database data: $dump_exit\e[0m"
        if [ "$VERBOSE" = "true" ]; then
            echo "$dump_output"
        fi
        return 1
    ...
    echo -e "\e[36m🔄 Importing data to local database...\e[0m"
    import_output=$(psql "postgresql://postgres:postgres@localhost:54322/postgres" -f supabase/data.sql 2>&1)
    import_exit=$?
    
    if [ $import_exit -ne 0 ]; then
        echo -e "\e[31m❌ Failed to import database data: $import_exit\e[0m"
        if [ "$VERBOSE" = "true" ]; then
            echo "$import_output"
        fi
        return 1
    ...
    echo -e "\e[32m✅ Database data initialized successfully\e[0m"
    update_config_value "dataInitialized" "true"
    return 0
}
...
echo -e "  --initData            Initialize database with data dump"
echo -e ""
echo -e "Force options (override completed steps):"
echo -e "  --force-install       Force npm install step"
echo -e "  --force-init          Force supabase init step"
echo -e "  --force-login         Force supabase login step"
echo -e "  --force-link          Force supabase link step"
echo -e "  --force-start         Force supabase start step"
echo -e "  --force-pull          Force database pull step"
echo -e "  --force-reset         Force database reset step"
echo -e "  --force-data          Force data initialization step"
echo -e "  --force-all           Force all steps to run"
echo -e ""
...
if ! invoke_step "Install Supabase" "npm install supabase --save-dev" "up to date" "installed" false "$FORCE_INSTALL"; then
    exit 1
fi

# Step 2: Initialize Supabase
if ! invoke_step "Initialize Supabase" "npx supabase init" "Finished supabase init" "initialized" false "$FORCE_INIT"; then
    exit 1
fi
...
if ! invoke_step "Login to Supabase" "npx supabase login --no-browser" "Happy coding" "loggedIn" "true" "$FORCE_LOGIN"; then
    exit 1
fi
...
if ! invoke_step "Link Supabase project" "$link_command" "Finished supabase link" "linked" "true" "$FORCE_LINK"; then
    exit 1
fi

# Step 6: Start Supabase
if ! invoke_step "Start Supabase" "npx supabase start" "Started supabase local development setup" "started" "true" "$FORCE_START"; then
    exit 1
fi
...
# Check if we need to pull the database schema
if [ "$(get_config_value "pulled")" = "true" ] && [ "$FORCE_PULL" = "false" ] && [ "$FORCE_ALL" = "false" ]; then
    echo -e "\e[32m✅ Step 'Pull database schema' already completed. Skipping...\e[0m"
else
    if [ "$FORCE_PULL" = "true" ] || [ "$FORCE_ALL" = "true" ]; then
        echo -e "\e[33m⚠️ Forcing database pull step to run even though it was previously completed.\e[0m"
    fi
...
if ! invoke_step "Reset local database" "npx supabase db reset --local" "Finished supabase db reset" "reset" false "$FORCE_RESET"; then
    exit 1
fi

# Step 9: Initialize data (optional)
if [ "$INIT_DATA" = true ]; then
    if [ "$(get_config_value "dataInitialized")" = "true" ] && [ "$FORCE_DATA" = "false" ] && [ "$FORCE_ALL" = "false" ]; then
        echo -e "\e[32m✅ Data initialization already completed. Skipping...\e[0m"
    else
        if [ "$FORCE_DATA" = "true" ] || [ "$FORCE_ALL" = "true" ]; then
            echo -e "\e[33m⚠️ Forcing data initialization step to run even though it was previously completed.\e[0m"
        fi
        
        if ! handle_data_init; then
            echo -e "\e[31m❌ Data initialization failed.\e[0m"
            exit 1
        fi
    fi
fi
